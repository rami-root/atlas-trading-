...
type StoredState = {
  ...
  balance: number;
  netProfits: number;
  deposits: Array<{ id: string; amount: number; walletAddress?: string; createdAt: string; status?: string; userName?: string; rejectReason?: string }>;
  withdrawals: Array<{
    id: string;
    userId?: number;
    userName?: string;
  ...
  users: Array<{ id: number; name: string; email: string; balance: number; role: 'user' | 'admin'; createdAt: string; referralCode?: string; referredBy?: string; referralEarnings?: number }>;
  tradingSettings: {
...
const fetchCoinGeckoPrices = async () => {
  try {
    ...
    console.error('Failed to fetch prices from server, using defaults:', error);
    // Return default prices if API fails
    return DEFAULT_CRYPTO_PRICES;
  }
};
...
const getDefaultState = (): StoredState => ({
  ...
  investments: [],
  users: [
    { id: 1, name: 'Admin', email: 'admin@example.com', balance: 0, role: 'admin', createdAt: new Date().toISOString(), referralEarnings: 0 },
    { id: 2, name: 'User', email: 'user@example.com', balance: 0, role: 'user', createdAt: new Date().toISOString(), referralEarnings: 0 },
  ],
  tradingSettings: {
...
export const trpcAdapter = {
      ...
      useMutation: () => {
        ...
        return {
          mutateAsync: async ({ amount, walletAddress }: { amount: string; walletAddress?: string }) => {
            ...
            // Don't call API for now, just update local state
            const state = loadState();

            // Resolve depositor from stored state (used for referral bonus chain)
            const depositor = user?.email ? state.users.find(u => u.email === user.email) : undefined;
            const depositorName = depositor?.name ?? 'User';
            const depositorReferralBy = depositor?.referredBy;

            // Multi-level referral bonus (credited immediately on deposit):
            // L1: 7%, L2: 2%, L3: 1%
            const awardBonus = (referralCode: string | undefined, percent: number) => {
              if (!referralCode) return;
              const refUser = state.users.find(u => u.referralCode === referralCode);
              if (!refUser) return;
              const bonus = amt * percent;
              refUser.referralEarnings = (refUser.referralEarnings ?? 0) + bonus;
            };

            // Level 1 referrer (the one whose code is in depositor.referredBy)
            awardBonus(depositorReferralBy, 0.07);

            // Level 2 referrer (referrer of the level 1 user)
            const level1User = depositorReferralBy
              ? state.users.find(u => u.referralCode === depositorReferralBy)
              : undefined;
            awardBonus(level1User?.referredBy, 0.02);

            // Level 3 referrer (referrer of the level 2 user)
            const level2User = level1User?.referredBy
              ? state.users.find(u => u.referralCode === level1User.referredBy)
              : undefined;
            awardBonus(level2User?.referredBy, 0.01);
  ...
  referral: {
    info: {
      useQuery: (input?: { userId?: string }, options?: { enabled?: boolean }) => {
        const { state } = useStoredState();
        const { user } = useAuth();
        ...
        // Find current user's referral code or generate one
        ...
        if (currentUser && currentUser.referralCode) {
          ...
          totalReferrals = state.users.filter(u => u.referredBy === currentUser.referralCode).length;
          totalEarnings = currentUser.referralEarnings ?? 0;
        } else {
          // Generate unique referral code for user
          referralCode = `ATLAS${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
          
          // Update user with referral code
          const userIndex = state.users.findIndex(u => u.email === user.email);
          if (userIndex !== -1) {
            state.users[userIndex].referralCode = referralCode;
            state.users[userIndex].referralEarnings = state.users[userIndex].referralEarnings ?? 0;
            saveState(state);
          }
        }
        ...
        return {
          data: {
            referralCode,
            referralLink: `${typeof window !== 'undefined' ? window.location.origin : 'http://localhost:5175'}/register?ref=${referralCode}`,
            totalReferrals,
    ...
    team: {
      useQuery: (input?: { userId?: string }, options?: { enabled?: boolean }) => {
        ...
        // Count actual referrals for current user
        let level1 = 0, level2 = 0, level3 = 0;
        
        const currentUser = state.users.find(u => u.email === user.email);
        if (currentUser && currentUser.referralCode) {
          // Count direct referrals (level 1)
          level1 = state.users.filter(u => u.referredBy === currentUser.referralCode).length;
          
          // Count level 2 referrals (referred by level 1 users)
          const level1Users = state.users.filter(u => u.referredBy === currentUser.referralCode);
          level1Users.forEach(level1User => {
            if (level1User.referralCode) {
              level2 += state.users.filter(u => u.referredBy === level1User.referralCode).length;
            }
          });
          
          // Count level 3 referrals (referred by level 2 users)
          const level2Users = state.users.filter(u => {
            return level1Users.some(l1 => l1.referralCode && u.referredBy === l1.referralCode);
          });
          level2Users.forEach(level2User => {
            if (level2User.referralCode) {
              level3 += state.users.filter(u => u.referredBy === level2User.referralCode).length;
            }
          });
        }
  ...
  admin: {
      ...
      useQuery: () => {
        ...
        return {
          data: {
            ...
            pendingDeposits: state.deposits.filter(d => (d as any).status === 'pending').length,
            pendingWithdrawals: state.withdrawals.filter(w => (w as any).status === 'pending').length,
    ...
    approveDeposit: {
      useMutation: () => {
        ...
        return {
          mutateAsync: async ({ depositId }: { depositId: string }) => {
            try {
              setIsPending(true);
    ...
    rejectDeposit: {
      useMutation: () => {
        const [isPending, setIsPending] = useState(false);
        return {
          mutateAsync: async ({ depositId, reason }: { depositId: string; reason: string }) => {
            try {
              setIsPending(true);
    ...
    withdrawals: {
      useQuery: () => {
        const { state, refetch } = useStoredState();
        const pending = state.withdrawals.filter(w => (w as any).status === 'pending');
        const data = pending.map((w, idx) => ({
          id: idx + 1,
          userId: (w as any).userId,
          userName: (w as any).userName,
          userEmail: (w as any).userEmail,
          amount: money(w.amount),
          fee: money((w as any).fee ?? 0),
          netAmount: money((w as any).netAmount ?? w.amount),
      ...
      useMutation: () => {
        return {
          mutateAsync: async ({ withdrawalId }: { withdrawalId: number }) => {
            const state = loadState();
            const pending = state.withdrawals.filter(w => (w as any).status === 'pending');
            const target = pending[withdrawalId - 1] as any;
    ...
    rejectWithdrawal: {
      useMutation: () => {
        return {
          mutateAsync: async ({ withdrawalId }: { withdrawalId: number; reason?: string }) => {
            const state = loadState();
            const pending = state.withdrawals.filter(w => (w as any).status === 'pending');
            const target = pending[withdrawalId - 1] as any;
            if (!target) throw new Error('Withdrawal not found');

            // Refund the amount back to profits (since we deducted it on create)
            state.netProfits += Number(target.amount ?? 0);
