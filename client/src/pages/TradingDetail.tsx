...
export default function TradingDetail() {
  ...
  useEffect(() => {
    ...
    // When directed trading is disabled, do not auto-fill the trade amount.
    // When directed trading is enabled, only auto-fill if the selected symbol matches.
    if (!didAutofill && isDirectedActive && isSymbolMatch) {
      const autoAmount = (capital.feeding * 0.15).toLocaleString('en-US', { minimumFractionDigits: 2 });
  ...
  useEffect(() => {
    ...
    // Keep stake at 15% of feeding (only in directed trading mode)
    if (capital?.feeding) {
      setAmount((capital.feeding * 0.15).toLocaleString('en-US', { minimumFractionDigits: 2 }));
    }
  ...
  const potentialProfit = (capital ? capital.feeding * 0.03 : 0).toLocaleString('en-US', { minimumFractionDigits: 2 });
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="container max-w-lg mx-auto pt-0 pb-2 px-4">
        ...
        <div className="bg-card border border-border rounded-lg p-3 mb-4 flex items-center justify-between">
          ...
          <div className={`text-sm font-medium ${(price?.change24h || 0) >= 0 ? 'text-green-500' : 'text-red-500'}`} style={{ fontFamily: 'monospace' }}>
            {(price?.change24h || 0) >= 0 ? '+' : ''}{price?.change24h.toLocaleString('en-US', { minimumFractionDigits: 2 })}%
          </div>
        </div>
