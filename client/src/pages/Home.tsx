  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="container max-w-lg mx-auto pt-0 pb-6 px-4">
        {/* عرض إجمالي الرصيد */}
        <div className="mb-8 bg-gradient-to-br from-blue-600 to-blue-800 rounded-2xl p-10 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-blue-100 text-xs">إجمالي تقييم الأصول</span>
          </div>
          <div className="flex items-baseline gap-3">
            <span className="text-white text-4xl font-bold" style={{ fontFamily: 'monospace' }}>
              {showBalance ? totalBalance.toLocaleString('en-US', { minimumFractionDigits: 2 }) : '****'}
            </span>
            <span className="text-blue-100 text-xl" style={{ fontFamily: 'monospace' }}>USDT</span>
          </div>
        </div>
          ...
          {prices?.map((crypto: any) => (
            ...
            >
              <div className="text-right">
                <div className="font-bold text-sm" style={{ fontFamily: 'monospace' }}>${crypto.price.toLocaleString('en-US', { minimumFractionDigits: 2 })} USDT</div>
                <div className={`text-xs flex items-center justify-end gap-1 ${crypto.change24h >= 0 ? 'text-green-500' : 'text-red-500'}`} style={{ fontFamily: 'monospace' }}>
