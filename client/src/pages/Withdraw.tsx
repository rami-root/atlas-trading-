...
export default function Withdraw() {
  ...
  const handleWithdraw = async () => {
    ...
    if (amountNum > maxWithdrawable) {
      toast.error(`لا يمكن سحب أكثر من ${maxWithdrawable.toLocaleString('en-US', { minimumFractionDigits: 2 })} USDT (الأرباح المتاحة)`);
      return;
    }
    ...
    try {
      await createWithdrawal.mutateAsync({ 
        amount, 
        walletAddress, 
        userId: user?.id.toString() || 'mock_user_1' 
      });
      toast.success(`تم إنشاء طلب السحب! ستستلم ${net.toLocaleString('en-US', { minimumFractionDigits: 2 })} USDT بعد خصم الرسوم`);
      setAmount('');
      setWalletAddress('');
    } catch (error: any) {
      ...
      <div className="container max-w-lg mx-auto py-6">
        ...
        <div className="stat-card mb-6">
          <div className="text-center">
            ...
            <div className="text-4xl font-bold text-primary" style={{ fontFamily: 'monospace' }}>
              {maxWithdrawable.toLocaleString('en-US', { minimumFractionDigits: 2 })} USDT
            </div>
        ...
        <div className="mb-6">
          ...
          />
          <div className="text-xs text-muted-foreground mt-2">
            الحد الأدنى: 10 USDT | الحد الأقصى: {maxWithdrawable.toLocaleString('en-US', { minimumFractionDigits: 2 })} USDT
          </div>
        </div>
          ...
          <div className="bg-secondary/50 border border-border rounded-lg p-4 mb-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-foreground font-medium" style={{ fontFamily: 'monospace' }}>{amountNum.toLocaleString('en-US', { minimumFractionDigits: 2 })} USDT</span>
                <span className="text-muted-foreground">المبلغ المطلوب:</span>
              </div>
              <div className="flex justify-between">
                <span className="text-red-500 font-medium" style={{ fontFamily: 'monospace' }}>-{fee.toLocaleString('en-US', { minimumFractionDigits: 2 })} USDT</span>
                <span className="text-muted-foreground">الرسوم (20%):</span>
              </div>
              <div className="border-t border-border pt-2 flex justify-between">
                <span className="text-primary font-bold" style={{ fontFamily: 'monospace' }}>{netAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })} USDT</span>
