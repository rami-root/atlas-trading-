      ...
      <div className="container max-w-lg mx-auto py-6">
        ...
        <div className="stat-card mb-6">
          <div className="text-center">
            ...
            <div className="text-4xl font-bold text-foreground mb-4" style={{ fontFamily: 'monospace' }}>
              {parseFloat(wallet?.totalBalance || '0').toLocaleString('en-US', { minimumFractionDigits: 2 })} USDT
            </div>
            
            <div className="grid grid-cols-2 gap-3 mt-4 text-center">
              <div className="bg-background/50 rounded-lg p-3">
                <div className="text-xs text-muted-foreground mb-1">إجمالي التغذية</div>
                <div className="text-sm font-semibold text-blue-500" style={{ fontFamily: 'monospace' }}>
                  {parseFloat((wallet as any)?.feedingBalance || wallet?.totalDeposits || '0').toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </div>
              </div>
              <div className="bg-background/50 rounded-lg p-3">
                <div className="text-xs text-muted-foreground mb-1">الأرباح</div>
                <div className="text-sm font-semibold text-green-500" style={{ fontFamily: 'monospace' }}>
                  {parseFloat(wallet?.netProfits || '0').toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </div>
              </div>
            </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-left">
                      <div className={`text-right font-medium ${
                        ...
                        {parseFloat(tx.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })} USDT
                      </div>
