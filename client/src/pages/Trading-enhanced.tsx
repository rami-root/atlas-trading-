...
export default function Trading() {
  ...
  // تحديث الأسعار كل 3 ثوانٍ للحصول على أسعار حية
  useEffect(() => {
  ...
  useEffect(() => {
    if (prices) {
      ...
      prices.forEach((crypto: any) => {
        const prevPrice = prevPricesRef.current[crypto.symbol];
        if (prevPrice !== undefined && prevPrice !== crypto.price) {
          newChanges[crypto.symbol] = crypto.price > prevPrice ? 'up' : 'down';
        }
        prevPricesRef.current[crypto.symbol] = crypto.price;
      });
      
      setPriceChanges(newChanges);
      
      // إزالة التأثيرات بعد ثانية واحدة
      setTimeout(() => {
        setPriceChanges({});
      }, 1000);
    }
        ...
        <div className="space-y-3">
          ...
          {isLoading ? (
            ...
            prices?.map((crypto: any) => {
              ...
              const bgColor = change === 'up' ? 'bg-green-500/10' : change === 'down' ? 'bg-red-500/10' : '';
              
              return (
                <div
                  key={crypto.symbol}
                  className={`bg-card border border-border p-4 rounded-xl cursor-pointer hover:bg-accent/50 transition-all duration-300 ${bgColor}`}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-right">
                      <div className={`font-bold text-foreground transition-all duration-300 ${
                        change === 'up' ? 'text-green-500 scale-105' : 
                        change === 'down' ? 'text-red-500 scale-105' : ''
